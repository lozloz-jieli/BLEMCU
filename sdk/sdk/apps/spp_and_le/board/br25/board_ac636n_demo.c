...
void sleep_enter_callback(u8  step)
{
    ...
    if (step == 1) {
        ...
        P33_CON_SET(P3_ANA_CON2, 1, 1, 0);
}


//-----------------------------------------------
extern void ble_module_enable(u8 en);
void board_ble_set_soft_poweroff(void)
{
	app_var.adv_close = 1;
    log_info("board_ble_set_soft_poweroff\n");

	power_wakeup_init(&wk_param);  // 重新设置唤醒源
...
{
    if(gpio_read(IO_PORTA_02) == 0){
        board_ble_set_soft_poweroff();
    }
}


static void power_io_pull_down_input(u8 key_io)
{
...
void board_power_io_init()
{
	u8 tempio;

	power_io_pull_down_input(IO_PORTA_02);
	os_time_dly(10);

	tempio = gpio_read(IO_PORTA_02);
	log_info("board_power_io_init reset_src: %d %d", power_reset_src,tempio);

	if((power_reset_src == 1) && (tempio == 0))
	{
		power_set_soft_poweroff();
	}

	port_wkup_interrupt_init(IO_PORTA_02, 1, app_set_soft_poweroff); //下降沿触发
}
