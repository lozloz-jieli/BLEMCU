...
#define LOG_TAG_CONST       APP_HaiEr  //保持与文件名同步
#define LOG_TAG             "[APP_HaiEr]"
...
extern void haier_write_buffer(void);
...
#if(TCFG_USER_EDR_ENABLE && UPLUS_TRANS_EN)
...
void app_set_test_poweroff(void)
{
	log_info("-----------------enter power off");
...
void app_set_soft_poweroff(void)
{
    ...
    //延时300ms，确保BT退出链路断开
    sys_timeout_add(NULL, app_set_test_poweroff, WAIT_DISCONN_TIME_MS);
}
...
static void app_start()
{
    ...
    log_info("btstack_init start");
    btstack_init();

    log_info("btstack_init end");
...
extern int haier_ct_dev_open(void);
...
//		haier_ct_dev_open();
        ...
        if (BT_MODE_IS(BT_BQB)) {
            ...
            log_info("haier_main is invoked.\n");
            haier_main();
        }
#endif
...
extern void ble_send_data_to_app_notify(void);
extern void ble_send_data_to_app_read(void);
...
u8 test_buffer[] = {0xff,0x00,0xff,0x00};
...
static void app_key_event_handler(struct sys_event *event)
{
    ...
    static u8 uplus_if_indicate_rate_test = 0;
    ...
    if (event->arg == (void *)DEVICE_EVENT_FROM_KEY) {
        ...
        if (event_type == KEY_EVENT_LONG && key_value == TCFG_ADKEY_VALUE6) {
            ...
            y_printf("******press(single click) TCFG_ADKEY_VALUE6*********\n");
            //app_set_soft_poweroff();
        }

        if (event_type == KEY_EVENT_CLICK && key_value == TCFG_ADKEY_VALUE0) {

            //ble_send_data_to_app_notify();
            y_printf("******press(single click) s201_key*********\n");
            //[haier sdk] bluetooth config start
            extern int uplus_ble_sdk_config_start(void);
            uplus_ble_sdk_config_start();
            y_printf("******uplus_ble_sdk_config_start*********\n");
            y_printf("******press(single click) s201_key end*********\n");
        }else if(event_type == KEY_EVENT_CLICK && key_value == TCFG_ADKEY_VALUE1){
            //ble_send_data_to_app_read();
            y_printf("******press(single click) s202_key*********\n");
            //[haier sdk] device into factory mode
            extern void  uplus_ble_sdk_recover_factory_mode(void);
            uplus_ble_sdk_recover_factory_mode();
            y_printf("******uplus_ble_sdk_recover_factory_mode*********\n");
            y_printf("******press(single click) s202_key end*********\n");
        }else if(event_type == KEY_EVENT_CLICK && key_value == TCFG_ADKEY_VALUE2){

            y_printf("******press(single click) s203_key*********\n");
#if 1
            extern uplus_user_ctrl_t g_user_ctrl_handle;
            //start indicate rate test
            if(!uplus_if_indicate_rate_test)
            {
                uplus_if_indicate_rate_test = 1;
                uplus_timer_start(UPLUS_TIMER_USER2);
                y_printf("******indicate rate test start, Time = %u(ms)*********\n", uplus_systick_uptime_get());
            ...
            {
                uplus_timer_stop(UPLUS_TIMER_USER2);
                y_printf("******indicate rate test stop, Time = %u(ms)*********\n", uplus_systick_uptime_get());
                uplus_if_indicate_rate_test = 0;
                g_user_ctrl_handle.uplus_indicate_send_data_num = 0;
            }

#endif
            y_printf("******press(single click) s203_key end*********\n");
        }else if(event_type == KEY_EVENT_CLICK && key_value == TCFG_ADKEY_VALUE3){
            y_printf("******press(single click) s204_key, event*********\n");
            //y_printf("led on or uplus get[%d]\n",uplus_gpio_in_get(&input_pin));
            //uplus_gpio_out_set(&output_pin,1);
            extern void uplus_user_rpt_event(void);
            uplus_user_rpt_event();
            y_printf("******press(single click) s204_key end*********\n");
        }else if(event_type == KEY_EVENT_CLICK && key_value == TCFG_ADKEY_VALUE4){
            y_printf("******press(single click) s205_key, status*********\n");
            //y_printf("led off or uplus get[%d]\n",uplus_gpio_in_get(&input_pin));
            //uplus_gpio_out_set(&output_pin,0);
            extern void uplus_user_rpt_status_extern();
            uplus_user_rpt_status_extern();
            y_printf("******press(single click) s205_key end*********\n");
        }else{
            //y_printf("ad_key no task!!!");
        }
        ...
        if (event_type == KEY_EVENT_TRIPLE_CLICK
            ...
            y_printf("******press(triple click) s201/s204_key*********\n");
            //app_set_soft_poweroff();
            y_printf("******press(triple click) s201/s204_key end*********\n");
            return;
        }

        if(event_type == KEY_EVENT_DOUBLE_CLICK){
			at_uart_send_packet(test_buffer,sizeof(test_buffer));
//			haier_write_buffer();
 ...
 * 注册AT Module模式
 */
REGISTER_APPLICATION(app_haier) = {
    .name 	= "haier",
    .action	= ACTION_UPLUS_MAIN,
    .ops 	= &app_spple_ops,
