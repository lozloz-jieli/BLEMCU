...
extern uart_bus_t *haier_uart_bus;
/* _WEAK_ */
/* extern */
...
static u8 ut_tx_pin[] = {
    IO_PORTA_05, IO_PORTB_00, IO_PORTB_05, IO_PORTC_03, //UT0_TX
    IO_PORTB_06, IO_PORTC_01, IO_PORTA_00, IO_PORT_DP,  //UT1_TX
    IO_PORTA_03, IO_PORTB_02, IO_PORTB_04, IO_PORTC_04, //UT2_TX
};
static u8 ut_rx_pin[] = {
    IO_PORTA_06, IO_PORTB_01, IO_PORTB_05, IO_PORTC_03, //UT0_RX
    IO_PORTB_07, IO_PORTC_02, IO_PORTA_01, IO_PORT_DM,  //UT1_RX
    IO_PORTA_04, IO_PORTB_03, IO_PORTB_04, IO_PORTC_05, //UT2_RX
};
...
u32 uart_dev_close(uart_bus_t *ut)
{
    ...
    if (&uart0 == ut) {
        UT0_close();
        g_printf("gpio_close_uart0\n");
        return 0;//gpio_close_uart0();
        // return gpio_close_uart0();
    } else if (&uart1 == ut) {
        UT1_close();
        g_printf("gpio_close_uart1\n");
        return 0;//gpio_close_uart0();
        // return gpio_close_uart1();
    } else {
        UT2_close();
        g_printf("gpio_close_uart2\n");
        return 0;//gpio_close_uart0();
...
/**
 * @description: 获取串口数据有没有发完
 * @param  {*}
 * @return {*}0：空闲   1：占用
 * @Author: jianye
 * @Date: 2021-08-16 20:24:20
 * @LastEditTime: Do not edit
 * @LastEditors: jianye
 * @param {uart_bus_t} *ut
 */
u32 uart_dev_busy(uart_bus_t *ut)  
{
    if (&uart0 == ut) {
        return (JL_UART0->CON0 & BIT(15));
    } else if (&uart1 == ut) {
        return (JL_UART1->CON0 & BIT(15));
    } else {
        return (JL_UART1->CON0 & BIT(15));
    }
}
...
void uart_dev_tx_enable(uart_bus_t *ut)
{
    if (&uart0 == ut) {
        JL_UART0->CON0 |= BIT(12) | BIT(10);
        JL_UART0->CON0 &= ~(BIT(5) | BIT(3) | BIT(1));
    } else if (&uart1 == ut) {
        JL_UART1->CON0 |= BIT(12) | BIT(10);
        JL_UART1->CON0 &= ~(BIT(5) | BIT(3) | BIT(1));
    } else {
        JL_UART2->CON0 |= BIT(12) | BIT(10);
        JL_UART2->CON0 &= ~(BIT(5) | BIT(3) | BIT(1));
    }
}

void uart_dev_tx_disable(uart_bus_t *ut)
{
    if (&uart0 == ut) {
        JL_UART0->CON0 |= BIT(12) | BIT(10);
        JL_UART0->CON0 |= BIT(5) | BIT(3) | BIT(1);
...
u8 haier_buffer[] = {0xff,0x00,0xff,0x00};
void haier_write_buffer(void)
{

	uart_dev_busy(haier_uart_bus);

	UT1_write_buf(haier_buffer, sizeof(haier_buffer));
}
