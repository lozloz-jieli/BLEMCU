...
extern void uplus_send_data(u8 *data);
volatile u8 uart_cmd_deal_busy = 0;
...
void at_cmd_rx_handler(void)
{
    ...
    if (*((u8 *)__this->pRxBuffer) != AT_PACKET_TYPE_CMD) {
		u8 num = 0;
    	u8	frame_len = 0;
		u8* pRxData = (u8*)__this->pRxBuffer; 
		
//		u32 cur_time = timer_get_ms();
//		if(uart_cmd_deal_busy == 0) {
//			uart_cmd_deal_busy = 1;
			
//			log_info("here is the poweroff0 the  0x%x, 0x%x, 0x%x ,num= %d",(pRxData[3] + 1),pRxData[4],(pRxData[5]),num);
//			switch(pRxData[4]){
//			
//				log_info("here is the poweroff2");
//				case 0x03:
//					if(pRxData[3] == ~pRxData[5]){
//					
//						log_info("here is the poweroff3");
//						board_ble_set_soft_poweroff();
//					}
//
//					break;
//			}
			if(pRxData[4] == 0x03){
				log_info("here is the poweroff2");
				if((pRxData[3] == 0xff) && (pRxData[5] == 0x00)){
					log_info("here is the poweroff3");
...
//        log_info("IS NOT TYPE_CMD");
        //uplus_send_data(__this->pRxBuffer);
        
        BP_FrameParse(__this->pRxBuffer,__this->data_length);
...
uart_bus_t *haier_uart_bus;

static int haier_ct_uart_init()
{
	g_printf("ct_uart_init");
    struct uart_platform_data_t u_arg = {0};
    u_arg.tx_pin = UART_DB_TX_PIN;
    u_arg.rx_pin = UART_DB_RX_PIN;
    u_arg.rx_cbuf = devBuffer_static;
    ...
    haier_uart_bus = uart_dev_open(&u_arg);

    if (haier_uart_bus != NULL) {
        log_info("uart_dev_open() success  0x%x\n",haier_uart_bus);
        __this->udev = haier_uart_bus;
        return 0;
    }
    return -1;
}
...
int haier_ct_dev_open(void)
{
	y_f_printf("ct_dev_open");
    haier_ct_uart_init();
    return 0;
}
